<script lang="ts">
  ...
  <div style="--btn-color: {color}" class="p-10 grid grid-cols-1 grid-rows-1 place-items-center">
    <button class="h-32 w-80 lg:flex-grow sm:flex-shrink rounded-full outline outline-10 unselectable" on:touchstart={handleMousedown} on:touchend={handleMouseup}>Defense</button>
  </div>
